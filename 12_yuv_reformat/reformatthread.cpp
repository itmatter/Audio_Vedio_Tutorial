...
#else
    #define IN_PCM_FILEPATH "/Users/lumi/Desktop/out.yuv"
    #define OUT_PCM_FILEPATH "/Users/lumi/Desktop/vyuy422_to_420p.yuv"
#endif



ReformatThread::ReformatThread(QObject *parent) : QThread(parent) {
    // 当监听到线程结束时（finished），就调用deleteLater回收内存
    connect(this, &ReformatThread::finished,
            this, &ReformatThread::deleteLater);
}

ReformatThread::~ReformatThread() {
    // 断开所有的连接
    disconnect();
    // 内存回收之前，正常结束线程
    requestInterruption();
